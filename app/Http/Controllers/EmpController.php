...
class EmpController extends Controller
{
    public function addEmployee()
    {
        ...
        return view('employee.add', compact('roles'));
    }
    ...
    public function showEmployee()
    {
        ...
        return view('employee.show_emp', compact('emps', 'column', 'string'));
    }
    ...
    public function showEdit($id)
    {
        ...
        return view('employee.add', compact('emps', 'roles'));
    }
    ...
    public function saveEdit_bankdetails(Request $request){
        ...
        $edit = Employee::where('user_id', $request->employee_id)->first();
        ...
        $edit->save();
        \Session::flash('flash_message', ' Employee Bank Details Updated Successfully.');
        return redirect()->back();
    }

    public function saveEdit_personal_detail(Request $request){
        $gender            = $request->gender;
        $dob               = date_format(date_create($request->date_of_birth), 'Y-m-d');
        $mob_number        = $request->number;
        $qualification     = $request->qualification;
        ...
        $edit->save();
        \Session::flash('flash_message', ' Employee Personal Details Updated Successfully.');
        return redirect()->back();
    }
    ...
    public function markAttendance()
    {
      ...
      $check=0;
      $inout=Attendance_Emp::where('user_id', Auth::user()->id)->whereDate('created_at', '=', Carbon::now()->format("Y-m-d"))->get();

      $check=Attendance_Emp::where('user_id', Auth::user()->id)->whereDate('created_at', '=', Carbon::now()->format("Y-m-d"))->count();
      ...
      if ($check=='1') {
        ...
        if(!empty($inout->clock_out)){

        return view('attendance.create_attendance',compact('check',json_encode($inout),'inout'));
        }
    ...
    public function processAttendance_in(Request $request)
    {

        $inout=new Attendance_Emp();
        $user = Auth::user();

        $inout->user_id=Auth::user()->id;
        $user->attendanceflag=0;
        $inout->flag=1;
        $inout->clock_in=Carbon::now();

        $user->save();
        $inout->save();
        \Session::flash('flash_message', 'Employee successfully clocked_in!');

        return redirect()->back();

    }

    public function processAttendance_out(Request $request)
    {
        $inout=Attendance_Emp::where('user_id', Auth::user()->id)->whereDate('created_at', '=', Carbon::now()->format("Y-m-d"))->first();
        $user = Auth::user();
        $inout->flag=2;
        $user->attendanceflag=1;
        Attendance_Emp::where('user_id',Auth::user()->id)->whereDate('created_at', '=', Carbon::now()->format("Y-m-d"))->update(array('clock_out' => Carbon::now()));


        $attendancehours = (Carbon::parse($inout->clock_in))->diffInHours((Carbon::parse($inout->clock_out)));
        $date_time1 = explode(' ', $inout->clock_in);
        $time1 = $date_time1[1];
        $temp='11:00:00';
        
          if(($attendancehours<='4') || ($time1>=$temp) )
          {
          $inout->day_status= "Half Day";
          }
          else {
          $inout->day_status= "Full Day";
          }
        $inout->save();
        $user->save();

        \Session::flash('flash_message', 'Employee successfully clocked_out!');
        return redirect()->back();
    }

    public function showAttendance()
    {
        $attendances=Attendance_Emp::where('user_id', Auth::user()->id)->get();


        return view('attendance.show_attendance_list', compact('attendances'));
    }
        ...
        foreach ($files as $file) {
            Excel::load($file, function ($reader) {
                ...
                foreach ($rows as $row) {
                    ...
                    $user->email    = str_replace(' ', '_', $row->emp_name) . '@taxivaxi.com';
                    $user->password = bcrypt('123456');
                    $user->save();
    ...
    public function showDetails()
    {
        ...
        return view('employee.show_bank_detail', compact('emps'));
    }
    ...
    public function doPromotion()
    {
        ...
        return view('promotion.add_promotion', compact('emps', 'roles'));
    }
    ...
    public function showPromotion()
    {
        ...
        return view('promotion.show_promotion', compact('promotions'));
    }
